...
# 페이지 기본 설정
st.set_page_config(page_title=" 경영제안서 생성기", layout="wide")
...
# ----------------------------------------------------------------------------------
# HTML 소스코드 (요청하신 자금 종류 레이아웃 완벽 적용)
# ----------------------------------------------------------------------------------
html_content = f"""
<!DOCTYPE html>
<html lang="ko">
<head>
    ...
    <style>
        ...
        body.force-print-mode .tab-content {{ display: block !important; opacity: 1 !important; page-break-after: always; }}
...
<body class="antialiased">
    ...
    <main class="max-w-5xl mx-auto px-6 py-8">
        ...
        <div id="tab_labor" class="tab-content print-active">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 print-break-inside-avoid">
                <div class="bg-white rounded-2xl border p-6">
                    ...
                    <div class="bg-blue-50 p-4 rounded-lg flex justify-between font-bold text-blue-700 text-xl">
                        <span>월 환산액 (월 209시간 기준)</span>
                        <span>{monthly_wage:,}원</span>
                    </div>
                    <p class="text-[11px] text-slate-400 mt-4 break-keep">※ 최저임금 위반 시 3년 이하 징역 또는 2천만원 이하 벌금이 부과되며, 별도의 시정지시 없이 즉시 처벌될 수 있습니다.</p>
                </div>
                <div class="bg-red-50 rounded-2xl border border-red-100 p-6 text-red-800 font-medium">
                    <h2 class="text-lg font-bold mb-2">🚨 근로계약서의 중요성 (사업주 관점)</h2>
            ...
            <div class="bg-white rounded-2xl border p-6 mb-8 print-break-inside-avoid">
                ...
                <table class="w-full text-[11px] text-left border-collapse border border-slate-200">
                    ...
                    <tbody class="divide-y divide-slate-200 text-slate-600">
                        <tr><td class="p-3 border font-bold">가산수당 (연장/야간/휴일)</td><td class="p-3 border">지급 의무 없음 (1배)</td><td class="p-3 border font-bold text-red-600">통상임금의 50% 가산 지급 (1.5배)</td></tr>
                        <tr><td class="p-3 border font-bold">연차유급휴가</td><td class="p-3 border">부여 의무 없음</td><td class="p-3 border font-bold text-red-600">법정 연차휴가 발생 (미사용 시 수당 정산)</td></tr>
                        <tr><td class="p-3 border font-bold">부당해고 구제신청</td><td class="p-3 border">적용 제외 (해고 비교적 자유)</td><td class="p-3 border font-bold text-red-600">노동위 구제신청 가능 (절차 엄격)</td></tr>
                        <tr><td class="p-3 border font-bold">주 52시간제</td><td class="p-3 border">미적용</td><td class="p-3 border font-bold text-red-600">주당 최대 52시간 이내 근로 준수 의무</td></tr>
                    </tbody>
                </table>
            </div>

            <div class="bg-emerald-50 rounded-2xl border border-emerald-100 p-6 print-break-inside-avoid">
                <h2 class="text-lg font-bold text-emerald-800 mb-4 border-b border-emerald-200 pb-2">💎 4대보험 비과세 항목 (합법적 소득 설계)</h2>
        ...
        <div id="tab_fixed" class="tab-content print-active">
            <h2 class="text-xl font-bold mb-4 border-l-4 border-amber-500 pl-2">💰 고용지원금 시뮬레이션 (총 {total_subsidy:,}만원 확보 예상)</h2>
            ...
            <div class="mb-10 print-break-inside-avoid">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div class="bg-emerald-50 p-5 rounded-xl border border-emerald-100 shadow-sm">
                        <h3 class="font-bold text-emerald-800 mb-2 font-black">1. 중소벤처기업진흥공단</h3>
                        ...
                        <ul class="text-xs text-emerald-700 space-y-1 break-keep">
                            ...
                            <li>• <b>특징:</b> 시중보다 저렴한 고정/변동 금리, 시설 및 운전자금 지원</li>
                        </ul>
                    </div>
                    <div class="bg-emerald-50 p-5 rounded-xl border border-emerald-100 shadow-sm">
                        <h3 class="font-bold text-emerald-800 mb-2 font-black">2. 신용/기술보증기금 (신보/기보)</h3>
                        <p class="text-sm text-emerald-700 font-bold mb-2 pb-2 border-b border-emerald-200">예상: 연 4.0% ~ 5.5% + 보증료</p>
                        <ul class="text-xs text-emerald-700 space-y-1 break-keep">
                            <li>• <b>성격:</b> 보증서 발급 대출 (보증료 별도)</li>
                            <li>• <b>특징:</b> 기술 등급 및 신용도를 통한 1금융권 대출 연계</li>
                        </ul>
                    </div>
                    <div class="bg-emerald-50 p-5 rounded-xl border border-emerald-100 shadow-sm">
                        <h3 class="font-bold text-emerald-800 mb-2 font-black">3. 일반 시중 은행</h3>
                        ...
                        <ul class="text-xs text-emerald-700 space-y-1 break-keep">
                            ...
                            <li>• <b>특징:</b> 재무제표 양호 시 빠른 한도 산출</li>
                        </ul>
                    </div>
                    <div class="bg-emerald-50 p-5 rounded-xl border border-emerald-100 lg:col-span-3 shadow-sm">
                        <h3 class="font-bold text-emerald-800 mb-2 font-black">4. 지자체 정책자금 (도 자금 / 시 자금 이차보전)</h3>
                        <p class="text-sm text-emerald-700 font-bold mb-2 pb-2 border-b border-emerald-200">실질 체감 금리: 연 1.5% ~ 3.5% 수준</p>
                        <div class="flex flex-col md:flex-row gap-4 mt-2">
                            <ul class="text-xs text-emerald-700 space-y-1 flex-1">
                                <li>• <b>성격:</b> 시중 은행 이자 중 일부를 지자체가 직접 대납</li>
                                <li>• <b>지원율:</b> 통상 은행 금리에서 1.0% ~ 3.0%를 도/시에서 납부</li>
                            </ul>
                            <ul class="text-xs text-emerald-700 space-y-1 flex-1 font-bold">
                                <li>• <b>경기도 자금:</b> 경기도 중소기업 육성자금 (시설/운전)</li>
                                <li>• <b>시 자금:</b> 시흥시 등 관내 중소기업 육성자금 (조기신청 필수)</li>
                            </ul>
                        </div>
                    </div>
                </div>
            ...
            <h2 class="text-xl font-bold mb-4 border-l-4 border-blue-500 pl-2">🏆 핵심 인증 리스트 (요건 분석)</h2>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
                <div onclick="toggleCert(this)" class="cert-card cursor-pointer bg-white p-4 rounded-xl border border-slate-200 shadow-sm transition-all hover:bg-blue-50 relative">
                    ...
                    <h3 class="font-bold text-sm mb-1">여성기업인증</h3><p class="text-[10px] text-slate-500">수의계약 한도 확대 / 입찰 가점</p>
                </div>
                <div onclick="toggleCert(this)" class="cert-card cursor-pointer bg-white p-4 rounded-xl border border-slate-200 shadow-sm transition-all hover:bg-blue-50 relative">
                    <div class="cert-badge absolute right-0 top-0 bg-blue-600 text-white text-[10px] font-bold px-2 py-1 rounded-bl-lg hidden">✔️ 대상</div>
                    <h3 class="font-bold text-sm mb-1">ISO 9001/14001/45001</h3><p class="text-[10px] text-slate-500">품질/환경/안전 글로벌 표준 인증</p>
                </div>
                <div onclick="toggleCert(this)" class="cert-card cursor-pointer bg-white p-4 rounded-xl border border-slate-200 shadow-sm transition-all hover:bg-blue-50 relative">
                    <div class="cert-badge absolute right-0 top-0 bg-blue-600 text-white text-[10px] font-bold px-2 py-1 rounded-bl-lg hidden">✔️ 대상</div>
                    <h3 class="font-bold text-sm mb-1">메인비즈 / 이노비즈</h3><p class="text-[10px] text-slate-500">기술 및 경영 혁신형 기업 인증</p>
                </div>
            </div>
        </div>
        ...
        <div id="tab_schedule" class="tab-content print-active">
            <div class="bg-white rounded-2xl border p-8 print-break-inside-avoid">
                ...
                <div class="space-y-8">
                    <div class="flex gap-6 items-start"><div class="bg-slate-800 text-white px-3 py-1 rounded font-bold text-sm">{m1:02d}월</div><div><p class="font-bold text-lg">인사/노무 기반 정비</p><p class="text-xs text-slate-500">근로계약서, 취업규칙 정비 및 비과세 설계</p></div></div>
                    <div class="flex gap-6 items-start"><div class="bg-blue-600 text-white px-3 py-1 rounded font-bold text-sm">{m1:02d}~{m3:02d}월</div><div><p class="font-bold text-lg">핵심 기업인증 확보</p><p class="text-xs text-slate-500">메인/이노비즈, ISO, 특허 집중 가동</p></div></div>
                    <div class="flex gap-6 items-start"><div class="bg-emerald-600 text-white px-3 py-1 rounded font-bold text-sm">{m3:02d}~{m4:02d}월</div><div><p class="font-bold text-lg">법인전환 및 자금 유치</p><p class="text-xs text-slate-500">절세 기반 전환 및 기관별 정책자금 매칭</p></div></div>
                </div>
            </div>
        </div>
    </main>
