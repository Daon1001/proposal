...
# 세금 시뮬레이션 계산 (변수명 NameError 방지용 통일)
p_total = (net_inc * 0.35 * 1.1) + (net_inc * 0.07) # 종소세 + 지역건보
c_total = (max(0, net_inc - salary*12) * 0.1 * 1.1) + (salary*12 * 0.15 * 1.1) + (salary*12 * 0.18) # 법인세 + 근로세 + 직장보험
...
<body>
    ...
    <nav class="no-print bg-white border-b sticky top-0 z-50 flex px-12 gap-10 shadow-sm overflow-x-auto">
        <button onclick="openTab(event, 'tab_proposal')" class="tab-btn py-5 text-base font-bold text-blue-600 border-b-4 border-blue-600 whitespace-nowrap">1. 핵심제안</button>
        <button onclick="openTab(event, 'tab_labor')" class="tab-btn py-5 text-base font-bold text-slate-500 border-b-4 border-transparent whitespace-nowrap">2. 노무/비과세</button>
        {'<button onclick="openTab(event, \'tab_corp\')" class="tab-btn py-5 text-base font-bold text-slate-500 border-b-4 border-transparent whitespace-nowrap">3. 법인전환</button>' if show_corp else ''}
        <button onclick="openTab(event, 'tab_fixed')" class="tab-btn py-5 text-base font-bold text-slate-500 border-b-4 border-transparent whitespace-nowrap">4. 인증/지원금</button>
        <button onclick="openTab(event, 'tab_fund')" class="tab-btn py-5 text-base font-bold text-slate-500 border-b-4 border-transparent whitespace-nowrap">5. 정책자금</button>
        <button onclick="openTab(event, 'tab_schedule')" class="tab-btn py-5 text-base font-bold text-slate-500 border-b-4 border-transparent whitespace-nowrap">6. 스케쥴</button>
    </nav>
    ...
    <main class="max-w-[1400px] mx-auto p-12">
        ...
        <div id="tab_labor" class="tab-content print-active">
            <div class="grid grid-cols-2 gap-10 mb-10">
                <div class="bg-white border border-blue-100 rounded-[32px] p-10 shadow-sm print-break-inside-avoid">
                    <h3 class="text-blue-700 font-bold text-xl mb-8 border-b pb-4">⏱️ 2026년 최저임금 기준</h3>
                    <div class="flex justify-between items-center mb-6">
                        <span class="text-slate-500 font-bold">2026 최저시급</span>
                    ...
                    <p class="text-xs text-slate-400 mt-6 leading-relaxed">※ 최저임금 위반 시 시정지시 없이 즉시 처벌 대상입니다.</p>
                </div>
                <div class="bg-slate-900 text-white rounded-[32px] p-10 shadow-sm print-break-inside-avoid">
                    <h3 class="text-amber-400 font-bold text-xl mb-8 border-b border-slate-700 pb-4">🚨 근로계약서 미작성 리스크</h3>
                    <ul class="space-y-6 text-base font-medium">
                        <li class="flex items-start gap-4">정규직 미작성: 벌금 최대 500만원 (전과 기록 남음)</li>
                        <li class="flex items-start gap-4">기간제/단시간 미작성: <b>과태료 500만원 즉시 부과</b></li>
                        <li class="flex items-start gap-4">분쟁 발생 시 계약서 부재는 사업주 100% 불리</li>
                    </ul>
                </div>
            </div>
            ...
            <div class="bg-white border border-slate-200 rounded-[32px] p-10 mb-10 shadow-sm print-break-inside-avoid">
                ...
                <table class="w-full text-base text-left border-collapse">
                    <thead class="bg-slate-50 text-slate-600 font-bold">
                        <tr><th class="p-5 border border-slate-200">구분 법령</th><th class="p-5 border border-slate-200 text-center">5인 미만</th><th class="p-5 border border-slate-200 text-center text-blue-600">5인 이상</th></tr>
                    </thead>
                    <tbody class="text-slate-700 font-medium">
                        <tr><td class="p-5 border border-slate-200 font-bold bg-slate-50/50">가산수당</td><td class="p-5 border border-slate-200 text-center">의무 없음</td><td class="p-5 border border-slate-200 text-center font-black text-red-600 text-xl">50% 가산 지급</td></tr>
                        <tr><td class="p-5 border border-slate-200 font-bold bg-slate-50/50">연차유급휴가</td><td class="p-5 border border-slate-200 text-center">부여 의무 없음</td><td class="p-5 border border-slate-200 text-center font-black text-red-600 text-xl">법정 연차 발생</td></tr>
                        <tr><td class="p-5 border border-slate-200 font-bold bg-slate-50/50">부당해고구제</td><td class="p-5 border border-slate-200 text-center">적용 제외</td><td class="p-5 border border-slate-200 text-center font-black text-red-600 text-xl">노동위 신청 가능</td></tr>
                    </tbody>
                </table>
            </div>

            <div class="bg-emerald-50 border border-emerald-100 rounded-[32px] p-10 print-break-inside-avoid">
                <h3 class="text-emerald-800 font-bold text-xl mb-8">💎 합법적 4대보험 비과세 설계 (매월 고정비 절감)</h3>
                <div class="grid grid-cols-4 gap-6 text-center">
                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-emerald-100"><p class="text-slate-500 font-bold mb-2 text-sm">식대</p><p class="text-2xl font-black text-emerald-600">20만</p></div>
                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-emerald-100"><p class="text-slate-500 font-bold mb-2 text-sm">자가운전</p><p class="text-2xl font-black text-emerald-600">20만</p></div>
                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-emerald-100"><p class="text-slate-500 font-bold mb-2 text-sm">보육수당</p><p class="text-2xl font-black text-emerald-600">20만</p></div>
                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-emerald-100"><p class="text-slate-500 font-bold mb-2 text-sm">연구보조</p><p class="text-2xl font-black text-emerald-600">20만</p></div>
                </div>
            </div>
        </div>
                ...
                <div class="grid grid-cols-2 gap-16 items-center">
                    <div class="space-y-8">
                        <div class="bg-red-50 p-8 rounded-[24px] border border-red-100 shadow-inner">
                            <p class="text-sm font-bold text-red-800 mb-2">개인 유지 시 비용 (종소세+지역건보)</p>
                        ...
                        <div class="bg-blue-50 p-8 rounded-[24px] border border-blue-100 shadow-inner">
                            <p class="text-sm font-bold text-blue-800 mb-2">법인 전환 시 통합 비용 (급여 포함)</p>
                        ...
                        <div class="bg-emerald-100 p-10 rounded-[24px] text-center border-2 border-emerald-200 shadow-lg">
                            <p class="text-4xl font-black text-emerald-900">💡 예상 절세액: <span class="text-6xl text-emerald-600">{max(0, tax_diff):,.0f}만 원</span></p>
                        </div>
                    </div>
        ...
        <div id="tab_fixed" class="tab-content print-active">
            <h2 class="text-2xl font-black mb-10 border-l-8 border-amber-400 pl-6">💰 고용지원금 시뮬레이션</h2>
            <div class="grid grid-cols-3 gap-8 mb-16">{cards_html}</div>
            <h2 class="text-2xl font-black mb-10 border-l-8 border-blue-500 pl-6">🏆 기업 핵심 인증 상세 혜택</h2>
            <div class="grid grid-cols-3 gap-8">
                <div class="bg-white border p-8 rounded-[24px] shadow-sm transition-all hover:bg-blue-50/30" onclick="this.classList.toggle('border-blue-500');this.classList.toggle('ring-4');this.classList.toggle('ring-blue-50')">
                    <h4 class="font-black text-lg mb-4 text-slate-800">여성기업인증</h4><p class="text-sm text-slate-500 font-medium">• 수의계약 한도 확대<br>• 공공입찰 가점</p>
                </div>
                <div class="bg-white border p-8 rounded-[24px] shadow-sm transition-all hover:bg-blue-50/30" onclick="this.classList.toggle('border-blue-500');this.classList.toggle('ring-4');this.classList.toggle('ring-blue-50')">
                    <h4 class="font-black text-lg mb-4 text-slate-800">직산 / 공장등록</h4><p class="text-sm text-slate-500 font-medium">• 조달청 참여 필수<br>• 제조업 세제 혜택</p>
                </div>
                <div class="bg-white border p-8 rounded-[24px] shadow-sm transition-all hover:bg-blue-50/30" onclick="this.classList.toggle('border-blue-500');this.classList.toggle('ring-4');this.classList.toggle('ring-blue-50')">
                    <h4 class="font-black text-lg mb-4 text-slate-800">ISO 9001 / 45001</h4><p class="text-sm text-slate-500 font-medium">• 글로벌 품질 표준<br>• 중대재해법 증빙</p>
                </div>
            </div>
        </div>

        <div id="tab_fund" class="tab-content print-active">
            <h2 class="text-2xl font-black mb-12 border-l-8 border-emerald-500 pl-6">🏦 기관별 정책자금 및 지원사업 가이드</h2>
            <div class="grid grid-cols-2 gap-12">
                <div class="bg-white border-t-8 border-emerald-500 p-10 rounded-[32px] shadow-lg print-break-inside-avoid">
                    ...
                    <ul class="text-base space-y-4 text-slate-600 font-bold leading-relaxed">
                        ...
                        <li>• <b>[바우처]</b> 최대 5,000만원 무상 지원</li>
                    </ul>
                </div>
                <div class="bg-white border-t-8 border-red-500 p-10 rounded-[32px] shadow-lg print-break-inside-avoid">
                    ...
                    <ul class="text-base space-y-4 text-slate-600 font-bold leading-relaxed">
                        <li>• <b>[보조금]</b> 고위험 설비 교체 최대 1억</li>
                        <li>• <b>[융자]</b> <b>연 1.5% 고정금리</b> (최대 10억)</li>
                    </ul>
                </div>
            </div>
        </div>
            ...
            <div class="bg-slate-900 text-white rounded-[40px] p-16 shadow-2xl">
                ...
                <div class="space-y-16">
                    <div class="flex gap-12 items-start">
                        ...
                        <div><p class="text-2xl font-black mb-3">인사/노무 정비 및 비과세 최적화</p><p class="text-slate-400 text-xl font-bold">근로계약서 개편 및 4대보험 고정비 절감 설계 완료</p></div>
                    </div>
                </div>
            </div>
        </div>
...
with col2:
    ...
    st.markdown(f'<a href="data:text/html;base64,{b64}" download="{client}_제안서.html" style="display: block; width: 100%; text-align: center; padding: 18px 0; background-color: #0F172A; color: white; text-decoration: none; border-radius: 12px; font-weight: bold; margin-top: 10px; font-size: 16px;">💾 제안서 파일 저장하기</a>', unsafe_allow_html=True)
